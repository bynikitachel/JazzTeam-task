...
const CellChange = ({ setIsChanged, cellValue, setCellValue }) => {
  ...
  return (
    <div className="container-change">
      <textarea autoFocus ref={textarea} value={newCellValue} onChange={(e) => setNewCellValue(e.target.value)} />
      <div className="container-textarea-buttons">
        <button className="cell-ok" onClick={() => handleOkChange()}>
          OK
        </button>
        <button className="cell-cancel" onClick={() => handleCloseChange()}>
          CANCEL
        </button>
      </div>
    </div>
